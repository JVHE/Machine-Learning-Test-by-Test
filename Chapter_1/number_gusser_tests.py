...
def given_one_datapoint_when_asked_to_guess_test():
    ...
    # then
    assert type(guessed_number) is int, 'the answer should be number'
...
def given_multiple_datapoints_when_asked_guess_many_times_test():
    # given
    number_guesser = NumberGuesser()
    previously_chosen_numbers = [1,2,5]
    number_guesser.numbers_were(previously_chosen_numbers)
    ...
    for guessed_number in guessed_numbers:
        assert guessed_number in guessed_numbers, 'every guess should be one of the previously chosen numbers'
        assert len(set(guessed_numbers)) > 1, "It should't always guess the same number."
...
def given_a_starting_set_of_observations_followed_by_a_one_off_observation_test():
    ...
    for guessed_number in guessed_numbers:
        ...
        assert len(set(guessed_numbers)) > 1, "It shouldn't alsays guess the same number."
